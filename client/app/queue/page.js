...
import { getQueList } from './queList';

export default function Queue() {

	const [queList, setQueList] = React.useState(null);

	React.useEffect(() => {
		async function fetchData() {
			const queList = await getQueList();
			setQueList(queList);
		}
		fetchData();
	}, []);
				...
				<div className='flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0'>
					<div className='relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg'>
						...
						<section className='flex flex-col items-center'>
							<h2 className='font-bold tracking-widest text-2xl'>
								{queList && queList.map((que) => (
									<div key={que.id}>{que.queNum}</div>
								))}
							</h2>
							<span className='text-sm leading-6 text-gray-600'>Current Number</span>

							<h2 className='pt-5 font-bold tracking-widest text-2xl'>7323</h2>
							<span className='text-sm leading-6 text-gray-600'>Your Number</span>
						</section>
